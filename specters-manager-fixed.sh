...
# =============================================================================
# Specters Application Manager (Windows-Compatible Version)
...
show_banner() {
    ...
    echo "║                  (Version Windows-Compatible)               ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
check_prerequisites() {
    ...
    local max_version="21.0.0"
    
    # Vérifier la version Node.js avec une approche plus robuste
    if [[ $(echo "$node_version" | cut -d. -f1) -ge 24 ]]; then
        log "WARN" "Node.js version $node_version détectée. Version recommandée: 20.x.x"
        log "WARN" "L'application peut fonctionner mais des avertissements peuvent apparaître"
...
# Fonction Windows-compatible pour vérifier les ports
wait_for_port() {
    local port=$1
    ...
    while [[ $count -lt $timeout ]]; do
        # Utiliser PowerShell pour tester la connexion (compatible Windows)
        if powershell.exe -Command "Test-NetConnection -ComputerName localhost -Port $port -InformationLevel Quiet" 2>/dev/null | grep -q "True"; then
    ...
    log "ERROR" "Timeout: $service_name n'est pas disponible sur le port $port après ${timeout}s"
    return 1
}

check_port_available() {
    local port=$1
    # Utiliser PowerShell pour vérifier si le port est libre
    if powershell.exe -Command "Test-NetConnection -ComputerName localhost -Port $port -InformationLevel Quiet" 2>/dev/null | grep -q "True"; then
...
start_docker_services() {
    ...
    # Attendre que les services soient prêts avec des timeouts plus longs
    log "INFO" "Attente de l'initialisation des services Docker..."
    sleep 5  # Laisser le temps aux conteneurs de démarrer
    
    if wait_for_port 5432 "PostgreSQL" 45; then
        log "SUCCESS" "PostgreSQL est prêt"
    else
        log "WARN" "PostgreSQL semble ne pas répondre, mais on continue..."
    fi
    
    if wait_for_port 6379 "Redis" 30; then
        log "SUCCESS" "Redis est prêt"
    else
        log "WARN" "Redis semble ne pas répondre, mais on continue..."
    fi
...
install_dependencies() {
    ...
    # Supprimer les warnings Node.js en définissant la variable d'environnement
...
start_backend() {
    ...
    # Attendre que le backend soit prêt avec un timeout plus long
    if wait_for_port 3000 "Backend" 90; then
        log "SUCCESS" "Backend démarré (PID: $backend_pid)"
        return 0
    else
        log "ERROR" "Échec du démarrage du backend"
        log "INFO" "Vérifiez les logs: $SCRIPT_DIR/logs/backend.log"
        return 1
    fi
}
...
start_frontend() {
    ...
    if wait_for_port 4200 "Frontend" 90; then
        log "SUCCESS" "Frontend démarré (PID: $frontend_pid)"
        return 0
    else
        log "ERROR" "Échec du démarrage du frontend"
        log "INFO" "Vérifiez les logs: $SCRIPT_DIR/logs/frontend.log"
        return 1
    fi
}
...
start_all() {
    ...
    echo -e "${YELLOW}   - Utilisez './specters-manager-fixed.sh status' pour vérifier l'état${NC}"
    echo -e "${YELLOW}   - Utilisez './specters-manager-fixed.sh logs [service]' pour voir les logs${NC}"
...
show_help() {
    echo -e "${PURPLE}Specters Manager - Gestionnaire d'application (Version Windows-Compatible)${NC}"
    echo ""
    ...
    echo ""
    echo -e "${BLUE}Améliorations de cette version:${NC}"
    echo "  ✅ Compatible avec Windows/Git Bash"
    echo "  ✅ Utilise PowerShell pour la vérification des ports"
    echo "  ✅ Gestion améliorée des erreurs"
    echo "  ✅ Timeouts plus longs pour le démarrage"
    echo "  ✅ Logs détaillés dans le dossier ./logs/"
}
...
main() {
    ...
    echo "=== Specters Manager (Fixed) - $(date) ===" >> "$LOG_FILE"
